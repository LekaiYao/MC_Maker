...
def main() -> None:
    ...
    processes = variables["NTUPLE_PROCESSES"].split()

    logs_dir = BASE_DIR / "NTUPLE" / "logs"
    submit_dir = BASE_DIR / "NTUPLE" / "submit"
    executable = BASE_DIR / "bin" / "run_ntuple_input_condor.sh"
    ...
    jobs = collect_jobs_from_manifest(
        ...
        input_base=Path(variables["MINIAOD_OUTPUT_BASE"]),
        output_base=Path(variables["NTUPLE_OUTPUT_BASE"]),
