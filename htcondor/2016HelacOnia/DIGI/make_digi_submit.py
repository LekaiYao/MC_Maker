...
def main() -> None:
    ...
    processes = variables["DIGI_PROCESSES"].split()

    logs_dir = BASE_DIR / "DIGI" / "logs"
    submit_dir = BASE_DIR / "DIGI" / "submit"
    executable = BASE_DIR / "bin" / "run_digi_input_condor.sh"
    ...
    jobs = collect_jobs_from_manifest(
        ...
        input_base=Path(variables["SIM_OUTPUT_BASE"]),
        output_base=Path(variables["DIGI_OUTPUT_BASE"]),
