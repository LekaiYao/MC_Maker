...
SINGLE_SUBMIT="${BASE}/DAG/submit_part_run_dag.sh"
...
if [ ! -x "$SINGLE_SUBMIT" ]; then
  echo "ERROR: single part-run DAG submit script is not executable: $SINGLE_SUBMIT"
  exit 1
fi

echo "Batch part-run DAG submit:"
