...
def main() -> None:
    ...
    processes = variables["SIM_PROCESSES"].split()

    logs_dir = BASE_DIR / "SIM" / "logs"
    submit_dir = BASE_DIR / "SIM" / "submit"
    executable = BASE_DIR / "bin" / "run_sim_input_condor.sh"
    ...
    jobs = collect_jobs_from_manifest(
        ...
        input_base=Path(variables["GEN_OUTPUT_BASE"]),
        output_base=Path(variables["SIM_OUTPUT_BASE"]),
