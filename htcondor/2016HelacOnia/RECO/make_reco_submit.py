...
def main() -> None:
    ...
    processes = variables["RECO_PROCESSES"].split()

    logs_dir = BASE_DIR / "RECO" / "logs"
    submit_dir = BASE_DIR / "RECO" / "submit"
    executable = BASE_DIR / "bin" / "run_reco_input_condor.sh"
    ...
    jobs = collect_jobs_from_manifest(
        ...
        input_base=Path(variables["HLT_OUTPUT_BASE"]),
        output_base=Path(variables["RECO_OUTPUT_BASE"]),
